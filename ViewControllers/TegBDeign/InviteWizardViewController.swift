...
extension InviteWizardViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            guard let cell = tableViewForInvite.dequeueReusableCell(withIdentifier: R.reuseIdentifier.inviteWizedCell.identifier , for: indexPath) as? InviteWizardTablecell else {
                return UITableViewCell()
            }
            cell.configure(detail: user1[indexPath.row])
            return cell
        } else {
            guard let cell = tableViewForInvite.dequeueReusableCell(withIdentifier: R.reuseIdentifier.userTask.identifier, for: indexPath) as? InviteWizardUserTask else {
                return UITableViewCell()
            }
